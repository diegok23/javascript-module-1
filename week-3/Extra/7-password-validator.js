...
/* function validatePasswords(passwords) {
  let passVerified = [];
  passwords.map(function (item) {
    const format =
      /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9])(?!.*\s).{5,15}$/;
    const pass = format.test(item);
    passVerified.push(pass);
  });
  return passVerified;
} */

function validatePasswords(passwords) {
  const format = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9])(?!.*\s).{5,15}$/;
  return passwords.map((item) => format.test(item));
}
